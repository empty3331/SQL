...
SELECT nvl(manager_id,0)"관리자",
       to_char(avg(salary),'999999.9')"평균임금",
       max(salary)"최고임금",
       min(salary)"최저임금"   
FROM employees
GROUP BY nvl(manager_id,0)
HAVING  to_char(avg(salary),'999999.9')>=5000
ORDER BY to_char(avg(salary),'999999.9') DESC;
...
SELECT  hire_date,
        CASE WHEN hire_date<'02-12-31' THEN '창립멤버'
         WHEN hire_date>'02-12-31' and hire_date<='03-12-13' THEN '03년입사'
         WHEN hire_date>'03-12-31' and hire_date<='04-12-13' THEN '04년입사'
         ELSE '상장이후입사'
        END  optDate
FROM employees
